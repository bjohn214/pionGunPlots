...
fnames = glob.glob("/data/fmeloni/DataMuC_MuColl_v1/pionGun/reco/*.slcio")
#fnames = glob.glob("/data/fmeloni/DataMuC_MuColl_v1/pionGun_10/reco/*.slcio")
...
hists = {}
for obj in ["pfo", "pfo_pi", "mcp", "mcp_pi", "mcp_pi_match"]:
...
for f in fnames:
    ...
    for event in reader:
        ...
        for pfo in pfoCollection:
            ...
            if abs(pfo.getType())==211 or abs(pfo.getType())==111:
                hists["pfo_pi_pt"].Fill(pfo_tlv.Perp())
                hists["pfo_pi_eta"].Fill(pfo_tlv.Eta())
                hists["pfo_pi_phi"].Fill(pfo_tlv.Phi())
                n_pfo_pi += 1
                has_pfo_pi = True
                my_pfo_pi = pfo_tlv     # Storing this to use for matching in the next loop
        ...
        for mcp in mcpCollection:
            ...
            if abs(mcp.getPDG())==211 or abs(mcp.getPDG())==111 and mcp.getGeneratorStatus()==1:
                hists["mcp_pi_pt"].Fill(mcp_tlv.Perp())
                hists["mcp_pi_eta"].Fill(mcp_tlv.Eta())
                hists["mcp_pi_phi"].Fill(mcp_tlv.Phi())
                n_mcp_pi += 1
                ...
                if has_pfo_pi:
                    hists["mcp_pi_match_pt"].Fill(mcp_tlv.Perp())
                    hists["mcp_pi_match_eta"].Fill(mcp_tlv.Eta())
                    hists["mcp_pi_match_phi"].Fill(mcp_tlv.Phi())

                    hists["d_pi_dpt"].Fill(my_pfo_pi.Perp() - mcp_tlv.Perp())
                    hists["d_pi_drelpt"].Fill((my_pfo_pi.Perp() - mcp_tlv.Perp())/mcp_tlv.Perp())
                    hists["d_pi_deta"].Fill(my_pfo_pi.Eta() - mcp_tlv.Eta())
                    hists["d_pi_dphi"].Fill(my_pfo_pi.Phi() - mcp_tlv.Phi())
                    h_2d_relpt.Fill(mcp_tlv.Perp(), (my_pfo_pi.Perp() - mcp_tlv.Perp())/mcp_tlv.Perp())
        ...
        if n_pfo_pi > 1: print(n_pfo_pi)
        hists["mcp_n"].Fill(len(mcpCollection))
        hists["pfo_n"].Fill(len(pfoCollection))
        hists["mcp_pi_n"].Fill(n_mcp_pi)
        hists["pfo_pi_n"].Fill(n_pfo_pi)
        hists["mcp_pi_match_n"].Fill(n_pfo_pi)
...
print("\t%i pion MCPs"%hists["mcp_pi_pt"].GetEntries())
print("\t%i PFOs"%hists["pfo_pt"].GetEntries())
print("\t%i pion PFOs"%hists["pfo_pi_pt"].GetEntries())
...
for v in variables:
    ...
    eff = ROOT.TEfficiency(hists["mcp_pi_match_"+v], hists["mcp_pi_"+v])
    eff.Draw("ape")
    ROOT.gPad.Update()
    eff.SetLineWidth(2)
...
c.SaveAs("plots/d_pi_relpt_2d.png")
...
h_prof.Draw()
c.SaveAs("plots/d_pi_relpt_prof.png")
